...
* URL navigation in auth (#1603) (`20db27f`)
* check IndexedDB support (#1630) (`983d533`)
* show connected/connecting call status (`92f490e`)
* Disable webcam by default using callIntent='audio' (`99cae93`)
* Add privacy blur options in settings (`2056f05`)
* Add pronouns and timezone options along with arbitrary field rendering (`24ee52f`)
* Add Matrix widget support and integration manager (`72db176`)
* Update SVG files for cinny, cinny-highlight, and cinny-unread with new designs (`a0f7e5b`)
* make replies toggle replies (`5aeb8a9`)
* setting for hiding member events in read-only rooms (`7e2cb4e`)
* port MSC4140 scheduled messages from cinny#2671 (`7e22d86`)
* add Cloudflare Workers IaC and deployment workflows (`5bb0ce0`)
* quick react via message editor (`b281603`)
* add Show Unread Counts toggle to Visual Tweaks (`3a4961e`)
* cache image packs in localStorage to reduce sliding-sync dependency (`5c60536`)
...
* update dependency matrix-js-sdk to v24 (#1175) (`863612d`)
* Fixed small typo an cross signing reset modal (#1112) (`dcad184`)
* spoiler hidden link click (#1199) (`da92ce3`)
* update dependency matrix-js-sdk to v24.1.0 [security] (#1251) (`db33707`)
* login with sso when app using hash router (#1631) (`689adde`)
* update dependency katex to v0.16.10 [security] (#1654) (`b0796f7`)
* Fix video and audio loading with authenicated media (#1946) (`f2c31d2`)
* update dependency matrix-js-sdk to v34.8.0 (#2011) (`35b0b1e`)
* register service worker immediately and cache media requests (#1977) (`0e51e19`)
* update dependency matrix-js-sdk to v34.11.1 (#2053) (`492a149`)
* Prevent IME-exiting Enter press from sending message on Safari (#2175) (`867a472`)
* update dependency matrix-js-sdk to v38 [security] (#2493) (`f55a376`)
* show call nav status while active call is ongoing (`47f1d11`)
* clean up call nav/call view console warnings (`9dbe53a`)
* keep call media controls visible before joining (`7ceba03`)
* restore header icon button fill behavior (`4f498af`)
* update dependency folds to v2.5.0 (#2606) (`83e5125`)
* permissions and room icon resolution (#2) (`efb3e11`)
* preserve navigation context when opening non-call rooms (`d2348d2`)
* reset room name state when room instance changes (`23b1216`)
* update dependency react-router-dom to v6.30.3 (#2612) (`206a927`)
* align account-data and message edit payload typing (`346a32c`)
* decode route params before path regeneration (`bfd792e`)
* allow in-app notification in background when push notification isn't enabled (`7151597`)
* use unique tag for push notifications (`25d77d5`)
* load space list first (`6abd8ff`)
* only import matrix sdk types (`75816cf`)
* prevent sliding back after clicking push notification killing the pwa (`6a9fe21`)
* try-catch all badge API calls (`b56e793`)
* image not loading on mobile after lock/unlock (#2631) (`fbde1a2`)
* add noreferrer to sanitized links for improved privacy consistency (#2628) (`0f220f5`)
* align i18next with react-i18next (`a6a5573`)
* group duplicate conditions into one (`341e3cd`)
* typo (`cee5396`)
* adapt SmallWidget to matrix-widget-api 1.17.0 API (`a4c34ef`)
* render call room chat only when chat panel is open (`0f047e5`)
* show call settings permissions only for call rooms (`ff3def0`)
* use PhoneDown icon for hang up action (`5b7e96f`)
* enable members drawer toggle in desktop call rooms (`21e596d`)
* actually call the function to sync names (`6bd547d`)
* update dependency folds to v2.6.1 (#2679) (`1f03891`)
* delete and report button colors & missing modal backgrounds (`f4cc4d5`)
* replace --font-mono to make inline codeblocks use mono font (`9339af0`)
* semi-revert matrix-widget-api 1.17 bump and migrate to 1.13 API (`2b80df5`)
* wait for Element Call contentLoaded before widget handshake (`8421c0a`)
* reference Nunito Variable font (`f47824e`)
* add missing react hook dependencies (`600f22f`)
* restore pronouns component wrapper (`10d23b7`)
* address eslint no-console and no-void issues (`1eb477b`)
* break import-x/no-cycle dependency chains (`a486eb9`)
* resolve initMatrix eslint violations (`39c9d48`)
* render Text children explicitly in wrapper components (`3dec377`)
* focus nickname input imperatively in user chips (`017ba4f`)
* remove dead code and make eslint happy (`53c2ade`)
* remove unused vars from Message (`9f4204b`)
* drop arm/v7 from docker publish targets (`00009f7`)
* replace small html with sub (`3cf0e1f`)
* show highlight count on highlighted badges, total on standard (`497d74e`)
* fix room nav badge counts and add dot badge for non-notifying rooms (`7f5c9dd`)
* exclude DM rooms from space unread badge aggregation (`cabb65f`)
* preserve thread reply counts when clamping stale main-timeline total (`0d87c70`)
* improve DM name handling with nickname support (`494eeb7`)
* improve message send UX (`2cf5b83`)
* correct pending/failed message opacity levels (`d88682a`)
* make autocomplete detection split on any whitespace (`bf9b6f5`)
* disable auto-capitalization for server and username inputs (`049a12d`)
* iOS reliability, SW click handling, and account switching (`c996e5e`)
* improve unread count calculation and background session highlight badges (`c2a43ec`)
* prevent edit leakage, add required_state, re-render on Replace events, guard empty getContent() (`6dcaea6`)
